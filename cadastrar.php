...
//VERIFICAR SE A POSTAGEM EXISTE DE ACORDO COM OS CAMPOS DA QUI PRA BAIXO TUDO PEGO NO PHP MAILER
...
if(isset($_POST['nome_completo']) && isset($_POST['email']) && isset($_POST['senha']) && isset($_POST['repete_senha'])){
    ...
    if(empty($_POST['nome_completo']) or empty($_POST['email']) or empty($_POST['senha']) or empty($_POST['repete_senha']) or empty($_POST['termos'])){
        ...
        if($checkbox!=="ok"){
           ...
           //PARTE DE VERIFICAÇAO DOE EMAIL SE FOI CADASTRADO OU NÃO 
                ...
                if($sql->execute(array($nome,$email,$senha_cript,$recupera_senha,$token,$codigo_confirmacao,$status, $data_cadastro))){
                   ...
                   if($modo =="producao"){
                    ...
                    try {
                        ...
                        //RECEPÇAO 
                        $mail->setFrom('meusistema@hotmail.com', 'Cadastro do George'); //O MEU SISTEMA
                        ...
                        //CONTEUDO 
                         ...
                         $mail->Body    = '<h1>Por favor confirme seu e-mail abaixo:</h1><br><br><a style="background:green; 
                         color:white; text-decoration:none; padding:20px; border-radius:5px;" href=""'.$site.
                         '/confirmacao.php?cod_confirm='.$codigo_confirmacao.'">Confirmar E-mail</a>'; //CONTEUDO 
