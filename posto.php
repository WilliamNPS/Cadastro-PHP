...
<?php
       ...
       $sql = $pdo->prepare("SELECT * FROM usuarios ");
        ...
        $email = 'debora@hotmail.com';
        $sql->execute(array($email));
        $dados = $sql->fetchAll();
       
       
   
   //ORGANIZAR OS DADOS  DE ACORDO
   //CRESCENTE 
   $sql = $pdo->prepare("SELECT * FROM usuarios ORDER BY id ");

   ///LIMITAR A QUANTIDADE DE USUARIO NA TABELA
  // $sql = $pdo->prepare("SELECT * FROM usuarios ORDER BY id LIMIT 28,5");
   
  //DECRESCENTE
   // $sql = $pdo->prepare("SELECT * FROM usuarios ORDER BY  nome DESC");
   $sql->execute();
        $dados = $sql->fetchAll();
...
if (isset($_POST['salvar'])&& isset($_POST['nome'])&& isset($_POST['email'])){
    ...
    if(isset($_POST['deletar']) && isset($_POST['id_deleta']) && isset($_POST['nome_deleta']) && isset($_POST['email_deleta'])){
        ...
        //COMANDO PARA DELETAR       
        $sql = $pdo->prepare("DELETE FROM usuarios WHERE id=? AND nome=? AND email=?");
